...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #1) initializes hidden state
        out_affine, cache_affine = affine_forward(features, W_proj, b_proj)
        
        
        #2) word embedding (captions_in -> (word_embedd) -> vector of shape (N, T, W))
        out_embed, out_cache = word_embedding_forward(captions_in, W_embed)
        
        #3)RNN
        if (self.cell_type == "rnn"):
            out_h, cache_rnn = rnn_forward(out_embed, out_affine, Wx, Wh, b)
        elif (self.cell_type == "lstm"):
            out_h, cache_lstm = lstm_forward(out_embed, out_affine, Wx, Wh, b)
        
        #4) compute scores
        out_temp_affine, cache_temp_affine = temporal_affine_forward(out_h, W_vocab, b_vocab)

        #5)compare scores against ground truth. Mask as only some values are used (one-hot-encoding)
        loss, dx = temporal_softmax_loss(out_temp_affine, captions_out, mask, verbose=False)
        
        #======backprop========#
        
        dx, grads["W_vocab"], grads["b_vocab"] =  temporal_affine_backward(dx, cache_temp_affine)
        ...
        grads["W_embed"] = word_embedding_backward(dx, out_cache)
