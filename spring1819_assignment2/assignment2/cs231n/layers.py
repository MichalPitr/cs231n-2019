...
#                   calculate output as elementwise multiplication followed by a sum and adding bias
                    out[n,f, y, ww] = np.sum(padded_x[n,:,start_y:end_y, start_x:end_x]*w[f, :, :, :]) + b[f]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
#                     print(x[n, c, h:h+pool_height, w:w+pool_width])
...
#     for n in range(N):
#         for c in range(C):
#             out[n, c, :, :], cache = batchnorm_forward(x[n, c, :, :], gamma, beta, bn_param)
